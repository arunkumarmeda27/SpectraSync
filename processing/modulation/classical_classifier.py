...
class ClassicalModulationClassifier:
    ...
    def classify(cls, features: Dict[str, float]) -> List[Dict[str, Any]]:
        ...
        if abs_c20 > 0.45 and abs_c40 > 0.8:
            ...
            candidates.append(("BPSK", score, "High |C20| and large |C40| consistent with antipodal BPSK"))
        else:
        ...
        if abs_c20 < 0.4 and 0.4 <= abs_c40 <= 1.4 and abs_c42 > 0.6:
            ...
            candidates.append(("QPSK", score, "Low |C20|, moderate |C40|, and high |C42| matches 4-phase constellation"))
        else:
            candidates.append(("QPSK", 0.15, "Cumulant pattern outside typical QPSK boundaries"))

        # 3. 8-PSK Check: Small |C20|, very small |C40| (<0.4), and |C42| ~ 1.0
        if abs_c20 < 0.25 and abs_c40 < 0.35 and abs_c42 > 0.7:
            score = 0.85
            candidates.append(("8-PSK", score, "Suppressed |C20| and |C40| with unit |C42| indicates 8-PSK"))
        else:
            candidates.append(("8-PSK", 0.10, "Non-zero |C40| weakens 8-PSK hypothesis"))
        ...
        if var_env > 0.10 and 0.4 <= abs_c42 <= 0.9 and abs_c20 < 0.4:
            ...
            candidates.append(("16-QAM", score, "Multi-level amplitude variance and C42 kurtosis matches rectangular 16-QAM"))
        else:
            candidates.append(("16-QAM", max(0.05, var_env * 0.8), "Envelope variance too low for multi-amplitude QAM"))

        # 5. 64-QAM Check: Higher amplitude variance and smaller C42
        if var_env > 0.18 and 0.35 <= abs_c42 <= 0.75:
            score = 0.75
            candidates.append(("64-QAM", score, "Substantial amplitude variation consistent with dense QAM grid"))
        else:
        ...
        if var_env < 0.08 and sigma_af > 0.02:
            ...
            candidates.append(("2-FSK", score, "Constant envelope with distinct frequency modulation profile"))
            candidates.append(("4-FSK", score * 0.85, "Constant envelope multi-tone FSK candidate"))
        else:
            candidates.append(("2-FSK", 0.08, "Envelope variance indicates amplitude modulation"))
            candidates.append(("4-FSK", 0.05, "Envelope variance indicates amplitude modulation"))
